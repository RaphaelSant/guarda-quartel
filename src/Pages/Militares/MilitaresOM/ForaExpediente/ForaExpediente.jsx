...
export default function ForaExpediente() {
    const [militares, setMilitares] = useState([]);

    // Obter os dados dos civis no banco de dados e "setando" na useState civis
    const getMilitar = async () => {
        iniciarFirestoreDb();
        const db = getFirestore();
        const militaresCollectionRef = collection(db, "es_mil_fora_expediente");
        try {
            //const data = await getDocs(civisCollectionRef);
            const data = await getDocs(
                query(
                    militaresCollectionRef,
                    orderBy("dataEntrada"),
    ...
    // Hook para executar a função getMilitar() apenas uma vez, não sobrecarregando (leitura) no firestore.
    useEffect(() => {
        try {
            getMilitar();
    ...
    const deleteMilitar = async (id) => {
        ...
        const db = getFirestore();
        try {
            const militarlDoc = doc(db, "es_mil_fora_expediente", id);
    ...
    return (
        <>
            ...
            <h5 className="mt-4 mb-0 text-center d-print-none">
                Militares &gt; <strong style={{ color: "#008BD2" }}>Fora do expediente</strong>
            </h5>
                                ...
                                <tr key={militar.id} className="align-middle">
                                    ...
                                    <td className="d-print-none">
                                        <div className="d-flex align-items-center justify-content-center gap-3">
                                            <div>
                                                <Link to={"/militares/foraExpediente/editarRegistro/" + id}>
